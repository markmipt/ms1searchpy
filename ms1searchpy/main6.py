...
def process_peptides(args):
    ...
    if args['csd']:
        ...
        #expand each feature to 1 - 6 charge states
        mz_table = pd.concat(used_features.apply(expand_charges, args=(1, 6), axis=1).tolist())
        ...
        pfm_csd = used_features.reindex(df1['ids'])[['1', '2', '3', '4', '5', '6']].values
        ...
        unique_sequences[['1', '2', '3', '4', '5', '6']] = CSD_pred
        ...
        pfm_csd = np.concatenate([pfm_csd, (pfm_csd * np.arange(1, 7).reshape(1, 6)).sum(axis=1).reshape(-1, 1)], axis=1)
        pfm_csd_pred = np.concatenate([pfm_csd_pred, (pfm_csd_pred * np.arange(1, 7).reshape(1, 6)).sum(axis=1).reshape(-1, 1)], axis=1)

        #prediction error features for individual intensities and average charge
        z_deltas = pfm_csd - pfm_csd_pred
        df1[[f'z{z}_err' for z in '123456a']] = (z_deltas - z_deltas.mean(axis=0)) / z_deltas.std(axis=0).reshape(1, -1)
